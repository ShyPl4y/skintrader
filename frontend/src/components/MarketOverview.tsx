  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard label="Preço médio" value={stats ? `${stats.avg_min_price}€` : '—'} />
        <StatCard label="Mais cara" value={stats?.most_expensive?.name ? `${stats.most_expensive.price}€` : '—'} sub={stats?.most_expensive?.name} />
        <StatCard label="Maior volume" value={stats?.highest_volume?.name ? stats.highest_volume.volume?.toLocaleString() : '—'} sub={stats?.highest_volume?.name} />
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {spreads.map((s) => (
                ...
                >
                  <td className="py-2 px-2 text-zinc-100 font-medium truncate max-w-[240px]">{s.market_hash_name}</td>
